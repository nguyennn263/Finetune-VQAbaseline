...
class VietnameseVQADataset(Dataset):
    """Dataset class for Vietnamese VQA using CLIP and mT5"""
    
    def __init__(self, questions, image_dir, question_tokenizer, answer_tokenizer, clip_processor, max_length=128, transform=None):
        ...
        # Default image transform if none provided (CLIP will handle its own preprocessing)
    ...
    def __getitem__(self, idx):
        ...
        # Tokenize question with XLM-RoBERTa tokenizer
        ...
        # Tokenize answer with mT5 tokenizer
...
class VietnameseVQAModel(nn.Module):
    """Vietnamese VQA Model using CLIP, XLM-RoBERTa, and mT5"""
    ...
    def __init__(self, model_config):
        ...
        self.vision_model = CLIPVisionModel.from_pretrained(model_config['vision_model'])
        vision_dim = self.vision_model.config.hidden_size
        
        # Text encoder (XLM-RoBERTa)
        self.text_model = XLMRobertaModel.from_pretrained(model_config['text_model'])
        text_dim = self.text_model.config.hidden_size
        
        # Text decoder (mT5)
        self.decoder_tokenizer = AutoTokenizer.from_pretrained(model_config['decoder_model'], use_fast=False)
        self.text_decoder = T5ForConditionalGeneration.from_pretrained(model_config['decoder_model'])
...
class VQATrainer:
    ...
    def train_epoch(self):
        ...
        accumulation_steps = 4  # Simulate batch size 4 with gradient accumulation
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            total_loss += loss.item() * accumulation_steps  # Restore original loss for logging
    ...
    def evaluate(self):
        ...
        from cxmt5.model import compute_metrics
...
def prepare_data_from_dataframe(df):
    ...
    # Create questions list with all correct answers
    questions = []
    for idx, row in df.iterrows():
        ...
        questions.append({
            ...
            'image_id': row['image_id'],   # Keep original ID
            'image_link': row.get('image_link', ''),  # Keep original link
            'question': row['question'],
            'answers': answers_list,  # Keep all answers
